...
// 1. Tu colles tes anciens tableaux PHP ici
$actions2025 = [
    // SAMEDI 15 FÉVRIER
...
// 2. On reformate proprement en tableau associatif pour le JSON
$formattedActions2025 = array_map(fn ($act) => [
    'playerName' => $act[0],
    'points' => $act[1],
    'description' => $act[2],
    'dateAction' => $act[3].'T12:00:00Z', // Ajout d'une heure fictive pour le format ISO
], $actions2025);

$formattedActions2026 = array_map(fn ($act) => [
    'playerName' => $act[0],
    'points' => $act[1],
    'description' => $act[2],
    'dateAction' => $act[3].'T12:00:00Z',
], $actions2026);

// 3. On construit l'architecture finale de ton history.json
$export = [
    'admin' => [
        'username' => 'maxime',
    ...
    'competitions' => [
        [
            ...
            'referees' => ['Chloé'],
            'bonusDays' => [
                ['date' => '2025-02-18', 'multiplier' => 2],
                ['date' => '2025-02-20', 'multiplier' => 2],
            ],
            'actions' => $formattedActions2025,
        ],
        [
            ...
            'referees' => ['Axel'],
            'bonusDays' => [
                ['date' => '2026-02-26', 'multiplier' => 2],
            ],
            'actions' => $formattedActions2026,
        ],
    ],
];

// 4. LA MAGIE PHP : Conversion en JSON
// JSON_PRETTY_PRINT = Pour que ce soit lisible avec des retours à la ligne
// JSON_UNESCAPED_UNICODE = Pour garder les accents (é, à, etc.) intacts
$jsonContent = json_encode($export, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);

// 5. On sauvegarde le tout dans le fichier
$exportDir = __DIR__.'/import';

if (!is_dir($exportDir)) {
    mkdir($exportDir, 0777, true);
}

file_put_contents((string) $exportDir.'/history.json', $jsonContent);

echo "✅ Fichier history.json généré avec succès ! Vous pouvez le vérifier dans le dossier import.\n";
